...
Last updated: 28 July 2026 (multi-user identity migration)
...
| ✅ | **Multi-user login — unify the identity model** | Done 28 Jul 2026 — see the Shipped entry above for the full writeup. Sync/tokens now key off Supabase Auth `user.id`, existing data migrated, legacy password gate retired, frontend session bug found in live testing and fixed. |
| S | **Security hardening before sharing — RLS tightening** | The per-user usage limit half is shipped (see Shipped above). Still open: `supabase/rls-tightening.sql` is written but **not yet run** — it replaces the current wide-open `USING (true)` RLS policies with a real `auth.uid() = auth_user_id` check now that the identity migration makes that possible, closing the latent hole where anyone with the anon/publishable key could read/write any row directly. Defense-in-depth only (the app already scopes every query correctly via the service-role routes) but should be run before opening access up. Ready to run in Supabase SQL Editor any time. |
| S | **Verify with a genuinely second account** | Not yet done. Needs: a second real Supabase Auth account (separate email), ideally in an incognito window; confirm its tasks/profile/Gmail-or-Outlook connection are fully isolated from Teodora's; specifically re-test logging in as one account right after the other **on the same browser** (the exact scenario the old bug would have broken). Claude can't create the account or enter a password (credential-entry is off-limits) — this one needs Teodora to click through it, with Claude verifying the resulting data via read-only DB checks. |
