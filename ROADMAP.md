...
| S | **OneNote connection (read-only)** | Raised 28 Jul 2026. Scope: read-only, via Microsoft Graph (`Notes.Read`), same OAuth pattern as the existing Outlook integration — feeds OneNote content in as extra context for the brief/triage/context-survey, nothing written back. **Blocked on a prerequisite only Teodora can do**: the `Notes.Read` scope needs to be added to the Azure app registration (portal.azure.com → App registrations → API permissions) before any code against it can actually authenticate — not something to queue into an unattended overnight run until that's granted. |
