...
async function saveTokens(userId, provider, { accessToken, refreshToken, expiresAt, accountEmail }) {
  ...
  await supabase.from("oauth_tokens").upsert(row, { onConflict: "auth_user_id,provider" })
}
...
async function refreshMicrosoftAccessToken(refreshToken) {
  const res = await fetch("https://login.microsoftonline.com/consumers/oauth2/v2.0/token", {
    ...
    body: new URLSearchParams({
      ...
      refresh_token: refreshToken,
      grant_type: "refresh_token",
      // Must match the authorization scope in [...nextauth].js — an explicit
      // scope on a refresh request narrows the token to just what's listed
      // here, so a mismatch would silently drop Notes.Read on every refresh
      // even though the original consent granted it.
