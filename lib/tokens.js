...
async function refreshMicrosoftAccessToken(refreshToken) {
  ...
  // /common, matching the authorization endpoint in [...nextauth].js: a
  // work/school account's refresh token is rejected outright by /consumers
  // (that endpoint only knows about personal accounts), so a hardcoded
  // /consumers here would let those accounts connect and then break them an
  // hour later on the first refresh. /common redeems both kinds.
...
export async function revokeProviderToken(userId, provider) {
  ...
  try {
    ...
    // Microsoft has no public programmatic revoke for delegated tokens — the
    // closest equivalent is the person removing this app's consent themselves
    // (account.live.com/consent for personal accounts, myapps.microsoft.com for
    // work/school ones). We still delete our own stored copy either way (see
