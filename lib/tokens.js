...
// Thrown when the stored refresh token is dead for good — revoked by the user
// at the provider, expired after a long idle period, or invalidated by a
// password change — as opposed to a transient network/5xx blip. Nothing
// retries its way out of this: only a fresh consent will fix it. Callers turn
// it into a "reconnect" prompt rather than a generic 500, which is what the
// email routes used to return (with the raw upstream text) while Settings
// still cheerfully reported the provider as connected.
export class ReconnectRequiredError extends Error {
  constructor(provider, detail) {
    super(`${provider} connection needs reconnecting${detail ? `: ${detail}` : ""}`)
...
async function refreshGoogleAccessToken(refreshToken) {
  ...
  if (!res.ok) {
    ...
    // 400 invalid_grant is Google's answer for a refresh token that's been
    // revoked (myaccount.google.com → third-party access), gone stale after
    // ~6 months unused, or killed by a password change.
    if (res.status === 400 || res.status === 401) {
      throw new ReconnectRequiredError("google", body.slice(0, 200))
    }
...
async function refreshMicrosoftAccessToken(refreshToken) {
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => "")
    // Microsoft answers a withdrawn consent (account.live.com/consent) or an
    // expired refresh token with 400 invalid_grant, same as Google.
    if (res.status === 400 || res.status === 401) {
      throw new ReconnectRequiredError("microsoft", body.slice(0, 200))
    }
