...
// Short disambiguating hints for the two default categories that were
// getting confused for each other in practice (found 29 Jul 2026 — job
// applications and general "work admin" tasks both kept landing under
// "work", including things that clearly belonged in "applications").
// Only applies to these specific built-in ids; user-defined categories are
// just their own label, same as before.
const CATEGORY_HINTS = {
  applications: "job search specifically — applying, CVs, interviews, recruiter contact, application status updates",
  work: "work-related but NOT job search — current job admin, work projects, professional logistics",
}

export function categoryListStr(categories) {
  const cats = categories?.length ? categories : DEFAULT_CATEGORIES
  return cats.map((c) => `"${c.id}" (${c.label}${CATEGORY_HINTS[c.id] ? ` — ${CATEGORY_HINTS[c.id]}` : ""})`).join(", ")
}
...
// How long an overdue task keeps earning a place in the daily brief. It stays
// in the Tasks tab either way — this only stops it competing for one of the
// five or six brief slots. Without a cutoff the "was due" rule below kept
// resurfacing things forever: a lunch from nearly a month ago was still being
// reported as needing attention today, crowding out what actually did.
const STALE_OVERDUE_DAYS = 14

function daysAgo(dateISO, todayISO) {
  const ms = new Date(`${todayISO}T00:00:00Z`) - new Date(`${dateISO}T00:00:00Z`)
...
export async function runBriefPrompt(client, { tasks, context, categories }) {
  ...
  const pendingTasks = (tasks || [])
    ...
    // Long-stale items are dropped before the model ever sees them, rather
    // than left to the prompt's judgement — the instructions below explicitly
    // ask for "was due" tasks, so anything still sitting here at a month old
    // was being faithfully surfaced every single day.
    .filter((t) => {
      if (!t.date || t.date >= todayISO) return true
      // A past-dated event already happened — a lunch, an appointment. It was
      // never a thing to action, so unlike a genuinely overdue task it gets no
      // grace window at all.
      if (t.isEvent) return false
      return daysAgo(t.date, todayISO) <= STALE_OVERDUE_DAYS
    })
    .slice(0, 12)
    .map((t) => {
  ...
  // Lines she's dismissed off previous briefs. Fed back in as an explicit
  // exclusion list because the brief is regenerated from scratch each time —
  // hiding a line only in the UI meant the very next refresh produced the
  // same item again, often reworded just enough to slip past a text match.
  const dismissed = (context.dismissedBriefItems || []).filter(Boolean).slice(0, 40)
  const dismissedBlock = dismissed.length
    ? `\n${context.name} has explicitly dismissed these items from previous briefs. Do not include them again, and do not include a rephrased version of the same underlying thing:\n${dismissed.map((d) => `- ${d}`).join("\n")}\n`
    : ""
