...
// OneNote page text is cached in localStorage for 3h (see fetchOnenoteNotes).
// That cache is the only place note content is ever persisted — nothing is
// written to Supabase — so it's also the only thing that has to be cleaned up
// when the person using this device changes. It survived both sign-out and an
// account switch before this, leaving one person's actual note text sitting in
// the next person's browser storage.
function clearOnenoteCache() {
  if (typeof window === "undefined") return
  try {
    Object.keys(localStorage)
      .filter((k) => k.startsWith("nona_onenote_"))
      .forEach((k) => localStorage.removeItem(k))
  } catch {}
}

// Normalizes a rendered brief line into a stable dismissal key: drops the
// bullet, collapses whitespace, lowercases. The brief text is regenerated by
// the AI on every refresh and its wording, casing and punctuation wobble
// between runs, so matching dismissals on the exact string let a line she'd
// already removed come straight back looking almost identical.
function briefLineKey(line) {
  return String(line || "").replace(/^[•\-\s]+/, "").trim().toLowerCase().replace(/\s+/g, " ")
}
...
export default function Nona() {
  ...
  function handleAuthUser(user) {
    ...
    if (stored?.ownerId && stored.ownerId !== (user?.id || null)) {
      ...
      clearOnenoteCache()
      setOnboarded(false)
      // Brief dismissals ride along on the profile, so resetting it here also
      // clears one person's judgement about their own items rather than
      // leaking it to whoever logs in next on this device.
  ...
  // Non-fatal problems from the last inbox fetch: one provider failed while
  // the other returned mail. These used to be collected and then thrown away
  // unless *every* provider failed, so a broken Outlook connection showed a
  // perfectly normal-looking triage that was quietly missing half the inbox.
  const [emailWarning, setEmailWarning] = useState(null)
  // Providers whose stored grant the server reported as revoked/expired
  // (reconnectRequired). Drives the "Reconnect" prompts — oauth_tokens still
  // has a row, so provider-status.js alone would keep saying "connected".
  ...
  const [spendTotal, setSpendTotal] = useState(0) // net of returns
  const [spendCategories, setSpendCategories] = useState([]) // [{category, total}], biggest first
  const [spendRefunded, setSpendRefunded] = useState(0) // amount returned, shown so the net total isn't unexplained
  ...
  async function fetchSpendList(months = spendPeriod) {
    ...
    try {
      ...
      setSpendCategories(d.categories || [])
      setSpendRefunded(d.refunded || 0)
    } catch (e) {
  ...
  async function fetchEmails(force = false) {
    ...
    try {
      ...
      // Reads one provider's route, keeping the server's own error message
      // rather than the raw response body. These routes always answer with
      // JSON, so r.text() (what this used to do) produced things like
      // `Gmail: {"error":"Google token refresh failed: 400"}` on screen.
      async function loadFrom(provider, url, label) {
        ...
        if (d.reconnectRequired) needsReconnect[provider] = true
        fetchErrors.push(`${label}: ${d.error || r.statusText}`)
        console.error(`${label} fetch failed:`, r.status, d.error || "")
      }

      if (providers?.google) await loadFrom("google", "/api/email/gmail", "Gmail")
      // Outlook via Microsoft Graph OAuth (proper OAuth, replaces IMAP)
      if (providers?.microsoft) await loadFrom("microsoft", "/api/email/outlook", "Outlook")
      ...
      // One provider worked and another didn't. The triage below is still
      // worth showing, but it's built on a partial inbox — say so rather than
      // letting it read as a complete picture.
      if (fetchErrors.length > 0) {
        setEmailWarning(`Some mail couldn't be loaded — ${fetchErrors.join(" · ")}`)
      }
      ...
      const r = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            // Without this, dismissing a line only hid it until the next
            // regeneration rebuilt the brief from scratch and produced the
            // same item again.
  ...
  // Lines removed from the brief card. Kept on the profile rather than in
  // component state or a standalone localStorage key, for two reasons: the
  // brief text itself is cached for hours under "nona_brief", so state-only
  // dismissals vanished on every reload while the same cached brief came back
  // (which is what "I can't remove things from the brief" actually was); and
  // the profile is what /api/sync persists, so dismissals now follow her
  // across devices and the 7am push honours them too (see the cron's context).
  // Original text is stored, not the normalized key, so it stays readable when
  // handed back to the AI as an exclusion list.
  const dismissedBriefLines = profile.dismissedBriefItems || []
  const dismissedBriefKeys = useMemo(
    () => new Set(dismissedBriefLines.map(briefLineKey)),
    [dismissedBriefLines]
  )

  function dismissBriefLine(line) {
    const text = String(line || "").replace(/^[•\s]+/, "").trim()
    if (!text) return
    setProfile(p => {
      const prev = p.dismissedBriefItems || []
      if (prev.some(x => briefLineKey(x) === briefLineKey(text))) return p
      // Capped so the exclusion list handed to the AI can't grow without
      // bound; oldest dismissals fall off first.
      return { ...p, dismissedBriefItems: [...prev, text].slice(-60) }
    })
  }

  function restoreDismissedBriefLines() {
    setProfile(p => ({ ...p, dismissedBriefItems: [] }))
  }
              ...
              <div className="card" style={{ marginBottom: 20 }}>
                ...
                {briefLoading ? (
                  ...
                  <div style={{ fontSize: 14, lineHeight: 1.7, color: "var(--white)" }}>
                    {(() => {
                      // The visible lines are worked out up front rather than
                      // filtered inline, so a category header whose items have
                      // all been dismissed disappears with them instead of
                      // being left stranded above an empty section.
                      const entries = brief.split("\n")
                        ...
                        .map((line) => line.startsWith("•")
                          ? { kind: "item", line }
                          : { kind: "header", line })
                        .filter(e => e.kind !== "item" || !dismissedBriefKeys.has(briefLineKey(e.line)))
                      const kept = entries.filter((e, idx) => {
                        if (e.kind !== "header") return true
                        const next = entries.slice(idx + 1).find(x => x.kind === "header" || x.kind === "item")
                        return next?.kind === "item"
                      })

                      if (!kept.some(e => e.kind === "item")) {
                        return (
                          <div style={{ fontSize: 13, color: "var(--muted)" }}>
                            Nothing left in today's brief.
                            {dismissedBriefLines.length > 0 && (
                              <button className="btn-sm" style={{ marginLeft: 8 }} onClick={restoreDismissedBriefLines}>
                                Undo hidden ({dismissedBriefLines.length})
                              </button>
                            )}
                          </div>
                        )
                      ...
                      return kept.map((e, i) => e.kind === "header" ? (
                        <div key={`h${i}`} style={{ fontSize: 11, fontWeight: 700, color: "var(--gold)", textTransform: "uppercase", letterSpacing: "0.06em", marginTop: i === 0 ? 0 : 10, marginBottom: 2 }}>
                          {e.line}
                        </div>
                      ) : (
                        <div key={`i${i}`} style={{ display: "flex", alignItems: "flex-start", gap: 6 }} className="brief-line">
                          <div style={{ flex: 1 }}>{e.line}</div>
                          <button
                            className="brief-line-dismiss"
                            title="Remove this from the brief for good"
                            style={{ color: "var(--muted)", opacity: 0.35, fontSize: 14, lineHeight: 1, padding: "1px 3px", flexShrink: 0 }}
                            onClick={() => dismissBriefLine(e.line)}
                          >×</button>
                        </div>
                      ))
                    })()}
                    {dismissedBriefLines.length > 0 && (
                      <div style={{ marginTop: 10, paddingTop: 8, borderTop: "1px solid var(--border)", fontSize: 11, color: "var(--muted)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <span>{dismissedBriefLines.length} hidden</span>
                        <button className="btn-sm" style={{ fontSize: 11 }} onClick={restoreDismissedBriefLines}>Undo all</button>
                      </div>
                    )}
                  </div>
                ) : (
                  ...
                  <div className="card" style={{ marginBottom: 20 }}>
                    <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 12 }}>
                      <div>
                        ...
                        {/* The total is net of returns, so say so — otherwise
                            it silently disagrees with the sum of the cards. */}
                        {spendRefunded > 0 && (
                          <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>
                            after €{spendRefunded.toFixed(2)} returned
                          </div>
                        )}
                      </div>
                    ...
                    {spendCategories.length > 0 && (
                      <div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid var(--border)" }}>
                        {spendCategories.slice(0, 6).map((c) => {
                          const share = spendTotal > 0 ? Math.max(2, Math.round((c.total / spendTotal) * 100)) : 0
                          return (
                            <div key={c.category} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                              <div style={{ fontSize: 12, color: "var(--muted)", flex: "0 0 42%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                {c.category}
                              </div>
                              <div style={{ flex: 1, height: 5, borderRadius: 3, background: "var(--border)", overflow: "hidden" }}>
                                <div style={{ width: `${Math.min(100, share)}%`, height: "100%", background: "var(--gold)" }} />
                              </div>
                              <div style={{ fontSize: 12, color: "var(--white)", flex: "0 0 auto", minWidth: 54, textAlign: "right" }}>
                                €{c.total.toFixed(2)}
                              </div>
                            </div>
                          )
                      ...
                      <div className="spend-feed" style={{ height: 260 }}>
                        {spendItems.map((it) => (
                          <div key={it.id} className="spend-feed-card" style={{
                            ...
                            <span style={{ fontSize: 26 }}>{it.refunded ? "↩️" : "📦"}</span>
                            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--white)", lineHeight: 1.4, opacity: it.refunded ? 0.5 : 1 }}>{it.item_name}</div>
                            <div className="serif" style={{ fontSize: 22, color: it.refunded ? "var(--muted)" : "var(--gold)", textDecoration: it.refunded ? "line-through" : "none" }}>
                              {(it.currency === "EUR" ? "€" : it.currency === "GBP" ? "£" : it.currency === "USD" ? "$" : `${it.currency} `)}{Number(it.price).toFixed(2)}
                            </div>
                            {it.category && (
                              <div style={{ fontSize: 10, color: "var(--gold)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 700 }}>
                                {it.category}
                              </div>
                            )}
                            <div style={{ fontSize: 11, color: "var(--muted)" }}>
                              {it.refunded ? "Returned · " : ""}
            ...
            {tab === "mail" && <>
              ...
              {!providers?.google && !providers?.microsoft ? (
                ...
                <div className="card">
                  ...
                  {reconnectNeeded.google && <button className="btn-sm" onClick={() => signIn("google")}>Reconnect Gmail →</button>}
                  {reconnectNeeded.microsoft && <button className="btn-sm" onClick={() => signIn("microsoft")}>Reconnect Outlook →</button>}
                ...
                {emailWarning && (
                  <div className="card" style={{ borderColor: "rgba(232,122,122,0.3)", marginBottom: 12 }}>
                    <p style={{ fontSize: 12, color: "var(--muted)", margin: 0 }}>⚠ {emailWarning}</p>
            ...
            {tab === "settings" && <>
              ...
              {providers?.google ? (
                <div className="settings-row">
                  <div>
                    {/* A stored token row only proves she once connected, not
                        that the grant still works — if the last inbox fetch
                        came back reconnectRequired, saying "connected ✓" here
                        is the reason a revoked Gmail looked fine in Settings
                        while the Mail tab kept failing. */}
                    <div style={{ fontSize: 14 }}>{reconnectNeeded.google ? "⚠ Gmail needs reconnecting" : "Gmail connected ✓"}</div>
                    <div style={{ fontSize: 12, color: "var(--muted)" }}>
                      {reconnectNeeded.google ? "Access was revoked or expired — reconnect to keep reading your inbox" : providers.google.email}
                    </div>
                  </div>
                  {reconnectNeeded.google ? (
                    <button className="btn-sm" onClick={() => signIn("google")}>Reconnect →</button>
                  ) : (
              ...
              {providers?.microsoft ? (
                <div className="settings-row">
                  <div>
                    {/* Same as Gmail above — see that comment. */}
                    <div style={{ fontSize: 14 }}>{reconnectNeeded.microsoft ? "⚠ Outlook needs reconnecting" : "Outlook connected ✓"}</div>
                    <div style={{ fontSize: 12, color: "var(--muted)" }}>
                      {reconnectNeeded.microsoft ? "Access was revoked or expired — reconnect to keep reading your inbox" : providers.microsoft.email}
                    </div>
                  </div>
                  {reconnectNeeded.microsoft ? (
              ...
              {supabaseUser && (
                <div style={{ marginTop: 24 }}>
                  <button className="btn btn-outline" onClick={async () => {
                    // Note text is the one genuinely sensitive thing cached
                    // locally — don't leave it behind on a signed-out device.
                    clearOnenoteCache()
